...
//Projeto 9 – Escrever uma mensagem no display LCD 16 x 2
#include <Wire.h>
#include <EEPROM.h>
...
float valorAcumulado = EEPROM.read(valorAcumuladoEndereco) ; // Valor acumulado em ponto flutuante
...
void setup(){
  ...
  EEPROM.write(valorAcumuladoEndereco, valorAcumulado);// Carrega o valor acumulado da EEPROM ao iniciar
...
void loop() {
  ...
  if (LDR_10C < 500) {
    ...
    //EEPROM.write(valorAcumuladoEndereco, valorAcumulado); // Armazena o valor acumulado na EEPROM
  ...
  EEPROM.write(valorAcumuladoEndereco, valorAcumulado);
  float valor = EEPROM.read(valorAcumuladoEndereco);
  Serial.println(valorAcumulado);
